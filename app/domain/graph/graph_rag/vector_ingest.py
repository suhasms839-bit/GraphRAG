...
class VectorIngestionPipeline:
    ...
    async def ingest(self, text: str, filename: str, doc_id: int):
        ...
        if planned > max_per_ingest: raise ValueError("Ingest limit exceeded")
        if existing_chunks + planned > max_per_user: raise ValueError("User quota exceeded")
        if not langchain_docs: return None
        try:
            ...
            async def _bg_sync(docs, doc_id):
                try:
                    ...
                    if doc and ok:
                        ...
                        db.commit()
                    db.close()
                except Exception as e: logger.exception(f"BG sync failed: {e}")
